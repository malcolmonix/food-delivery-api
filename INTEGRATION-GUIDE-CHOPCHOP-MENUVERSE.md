...
All images are stored in Firebase Storage and returned as public URLs.

### Upload Generic Image

```graphql
mutation UploadImage($file: Upload!) {
  uploadImage(file: $file, folder: "custom-folder") 
}
```
...
```javascript
// Using Apollo Client with file uploads
...
const handleFileUpload = async (file) => {
  try {
    ...
    console.log('Image URL:', data.uploadImage);
  } catch (error) {
